...
const MovieInfo = ({ selectedMovieData, cancel }) => {
  ...
  return (
    <div className='w-[85%] md:left-24 mx-auto mt-3 p-5 box-border flex md:flex-row flex-col gap-5 bg-black sticky z-10  '>
      ...
      <div className="flex flex-col gap-5">
        ...
        <h3>Genre: <span className="opacity-70 bg-slate-300 p-1 rounded-lg text-black align-middle">{Genre}</span></h3>
          ...
          <>
            ...
            <ul>
              {Ratings.map((rating, index) => (
                <li key={index}>
                  {rating.Source}: <span className="opacity-70 bg-slate-300 p-1 rounded-lg text-black align-middle"> {rating.Value}</span>
                </li>
              ))}
            </ul>
          </>
      ...
      <FaRegWindowClose size={24} onClick={cancel} className='cursor-pointer'/>
    </div>
  );
};
